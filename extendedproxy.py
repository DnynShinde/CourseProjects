...
def cache_thread():
    while True:
        # Modify this part to cache specific pages as needed..
        path_to_cache = "/page_to_cache"
        server_add_to_cache = '192.168.135.246'
        server_port_to_cache = 15200

        if path_to_cache not in cache:
            # Create a Queue to store the result
            result_queue = Queue()

            # Create a thread to connect to the server
            server_thread = threading.Thread(
                target=handle_server, args=(path_to_cache, server_add_to_cache, server_port_to_cache, result_queue)
            )
            server_thread.start()
            ...
            response_to_cache = result_queue.get()

            # Cache the response
            cache[path_to_cache] = response_to_cache

# Create a separate thread for caching
caching_thread = threading.Thread(target=cache_thread)
caching_thread.daemon = True  # Allow the thread to exit when the main program exits
